...
export default function Extracurricular() {
  return (
    <section className="relative w-full max-w-6xl mx-auto py-24 px-4 sm:px-6" id="extracurricular">
      ...
      <div className="relative w-full max-w-5xl mx-auto bg-[#0a0f16]/40 backdrop-blur-md border border-white/10 rounded-3xl p-6 md:p-10 shadow-[0_0_30px_rgba(var(--color-accent-r),var(--color-accent-g),var(--color-accent-b),0.05)] z-10">
        {/* Inside Header */}
          ...
          {activitiesData.map((activity, idx) => {
            ...
            return (
              ...
              >
                ...
                {!isLast && (
                  <div className="absolute -left-[29px] md:-left-[45px] top-[32px] w-[16px] h-[calc(100%+32px)] border-x border-primary-500/30 z-0 overflow-hidden flex justify-center">
                    <motion.div
                ...
                <div className="absolute -left-[29px] md:-left-[45px] top-[24px] w-4 h-4 rounded-full bg-primary-400 shadow-[0_0_15px_rgba(var(--color-accent-r),var(--color-accent-g),var(--color-accent-b),1)] border-[3px] border-[#0a0e14] z-10 transition-transform duration-300 group-hover:scale-125" />

                {/* Connecting horizontal branch */}
                <div className="absolute -left-[13px] md:-left-[29px] top-[31px] w-[13px] md:w-[29px] h-[2px] bg-primary-500/30 shadow-[0_0_8px_rgba(var(--color-accent-r),var(--color-accent-g),var(--color-accent-b),0.5)] z-0" />

                {/* ── Inner Card ── */}
                <div className="bg-[#141923]/60 border border-[var(--color-primary-400)]/20 rounded-3xl p-6 md:p-8 shadow-[0_0_20px_rgba(var(--color-accent-r),var(--color-accent-g),var(--color-accent-b),0.15)] backdrop-blur-xl overflow-hidden hover:border-primary-500/50 transition-colors duration-300 relative">
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:justify-between gap-6 mb-8 border-b border-slate-800/80 pb-6 relative z-10">
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
                      ...
                      <div className="flex flex-col gap-1">
                        <h3 className="text-xl md:text-2xl font-bold text-white tracking-wide group-hover:text-primary-300 transition-colors">
                          {activity.role}
                        </h3>
                        <p className="text-sm md:text-base text-primary-400/90 font-medium">
                    ...
                    <div className="shrink-0 mt-2 sm:mt-0">
                      <span className="px-3 py-1 text-xs font-mono text-primary-300 bg-primary-500/10 border border-primary-500/20 rounded-full shadow-[0_0_10px_rgba(var(--color-accent-r),var(--color-accent-g),var(--color-accent-b),0.15)] inline-block tracking-wider">
                        {activity.period}
                      </span>
                    </div>
                  </div>
                  ...
                  <ul className="space-y-4 relative z-10">
                    {activity.details.map((point, i) => (
                      <li key={i} className="flex items-start gap-3 text-sm md:text-base text-slate-300 leading-relaxed">
                        <span className="w-1.5 h-1.5 rounded-full bg-primary-500/80 mt-2 flex-shrink-0 shadow-[0_0_8px_rgba(var(--color-accent-r),var(--color-accent-g),var(--color-accent-b),0.8)]" />
                        {point}
                      </li>
                    ))}
                  </ul>
